...
def sam_model_infer(model,
    ...
    with torch.no_grad():
        ...
        #np.save('low_res_masks.npy', low_res_masks.cpu().numpy())
    ...
    # np.save('medsam_seg_prob.npy', medsam_seg_prob.cpu().numpy())
    # np.save('flow_z.npy', flow_z.cpu().numpy())
    # np.save('flow_y.npy', flow_y.cpu().numpy())
    # np.save('flow_x.npy', flow_x.cpu().numpy())
    # !!! debug
    ...
    #np.save('medsam_seg_prob.npy', medsam_seg_prob)

    # print("!!! [Debug @sam_model_infer] medsam_seg_prob shape is", medsam_seg_prob.shape)  # (1, 64, 64, 64)
    ...
    print("!!! [Debug @sam_model_infer] medsam_seg_mask shape is", medsam_seg_mask.shape)  # (32, 32, 32)
...
def read_data_from_subject(subject):
    ...
    # np.save('img3D_roi_upscale.npy', img3D_roi_upscale.cpu().numpy())
...
def data_postprocess(roi_pred, meta_info, output_dir='outputs'):
    ...
    # !!! debugg
    print("roi_pred shape is", roi_pred.shape) # 128,128,128
