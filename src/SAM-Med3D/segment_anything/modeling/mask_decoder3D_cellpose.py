...
class MaskDecoder3DCellpose(nn.Module):
    def __init__(
        self,
        *,
        transformer_dim: int,
    ...
    def forward(
        ...
        if multimask_output:
            ...
            z_slice = slice(1, 2)
            y_slice = slice(2, 3)   
            x_slice = slice(3, None)
        #---------------------------------------------------
        # CHANGE
        # add flow prediction in decoder
        # --------------------------------------------------

        masks_prob = masks[:, mask_slice, :, :]
        flow_z = masks[:, z_slice, :, :]
        flow_y = masks[:, y_slice, :, :]
        flow_x = masks[:, x_slice, :, :]
        iou_pred = iou_pred[:, mask_slice]

        # Prepare output
        return masks_prob, flow_z, flow_y, flow_x, iou_pred
    ...
    def predict_masks(
        ...
        # flow_y = masks[:,2,:,:,:]
        # flow_x = masks[:,3,:,:,:]
        # print("!!! flow_z", flow_z.shape)  
        # !!! debug masks shape torch.Size([1, 4, 32, 32, 32])
        # !!! flow_z torch.Size([1, 32, 32, 32])
