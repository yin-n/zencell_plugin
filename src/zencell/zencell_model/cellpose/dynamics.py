...
def _extend_centers_gpu(
    neighbors, meds, isneighbor, shape, n_iter=200, device=None
):
...
def masks_to_flows_gpu(masks, device=None, niter=None):
    ...
    neighborsY = torch.stack(
        (y, y - 1, y + 1, y, y, y - 1, y - 1, y + 1, y + 1), dim=0
    )
    neighborsX = torch.stack(
        (x, x, x, x - 1, x + 1, x - 1, x + 1, x - 1, x + 1), dim=0
    )
...
def masks_to_flows_cpu(masks, device=None, niter=None):
    ...
    for i in prange(len(slices)):
        ...
        if si is not None:
            ...
            T = _extend_centers(
                T, y, x, ymed, xmed, np.int32(lx), np.int32(n_iter)
            )
...
def masks_to_flows(masks, device=None, niter=None):
    ...
    if masks.ndim == 3:
        ...
        for z in range(Lz):
            mu0 = masks_to_flows_device(masks[z], device=device, niter=niter)[
                0
            ]
            mu[[1, 2], z] += mu0
        for y in range(Ly):
            mu0 = masks_to_flows_device(
                masks[:, y], device=device, niter=niter
            )[0]
            mu[[0, 2], :, y] += mu0
        for x in range(Lx):
            mu0 = masks_to_flows_device(
                masks[:, :, x], device=device, niter=niter
            )[0]
            mu[[0, 1], :, :, x] += mu0
        return mu
    elif masks.ndim == 2:
...
def steps3D(p, dP, inds, niter):
    ...
    for t in range(niter):
        ...
        for j in range(inds.shape[0]):
            ...
            p[0, z, y, x] = min(
                shape[0] - 1, max(0, p[0, z, y, x] + dP[0, p0, p1, p2])
            )
            p[1, z, y, x] = min(
                shape[1] - 1, max(0, p[1, z, y, x] + dP[1, p0, p1, p2])
            )
            p[2, z, y, x] = min(
                shape[2] - 1, max(0, p[2, z, y, x] + dP[2, p0, p1, p2])
            )
    return p
...
def follow_flows(dP, mask=None, niter=200, interp=True, device=None):
    ...
    if len(shape) > 2:
        ...
        inds = (
            np.array(np.nonzero(np.abs(dP).max(axis=0) > 1e-3))
            .astype(np.int32)
            .T
        )
        ...
        if not interp:
            ...
            p_interp = steps2D_interp(
                p[:, inds[:, 0], inds[:, 1]], dP, niter, device=device
            )
...
def resize_and_compute_masks(
    ...
    if resize is not None:
        ...
        p = np.array(
            [
                transforms.resize_image(
                    pi, resize[0], resize[1], interpolation=cv2.INTER_NEAREST
                )
                for pi in p
            ]
        )
...
def compute_masks(
    ...
    if np.any(
        ...
        if p is None:
            p, inds = follow_flows(
                dP * cp_mask / 5.0, niter=niter, interp=interp, device=device
            )
            if inds is None:
        ...
        # calculate masks
        mask = get_masks(
            p, iscell=cp_mask, max_size_fraction=max_size_fraction
        )
